        ...
        if mode == "train":
            ...
            for epoch in range(self.max_epochs):
                ...
                for i, id_tensors in iter_data:
                    '''
                    target 都是 传入[B, L]好一点，算loss就只算[B, -1]
                    user_id: (b, 1)
                    input_item: (b, L)
                    input_beha: (b, L)
                    target_item: (b, L)
                    ...
                    # model_scores, diffu_rep, weights, t, item_rep_dis, seq_rep_dis =self.model(input_item, target_item, train_flag=True)
                    
                    # pos_ids 作为的target item 算的时候是最后一个，target beha是指序列每个位置对应的下一个行为
                    ...
                    seq_ce_loss = 0.0 # self.CELoss(seq_rep, pos_ids, test_item_emb)
                    # loss_diffu_value = self.model.loss_diffu_ce(diffu_rep, target_item) 计算方式同bce_loss，是等价的
                    
                    joint_loss = ce_loss + seq_ce_loss
                    self.optimizer.zero_grad()
                    joint_loss.backward()
                    self.optimizer.step()

                    ce_total_loss += ce_loss.item()
                    seq_ce_total_loss =0.0 # += seq_ce_loss.item()
            ...
            with torch.no_grad():
                for i, id_tensors in iter_data:
                    ...
                    # scores_rec, rep_diffu, _, _, _, _ = self.model(input_item, None, train_flag=False) # TODO tag部分应该设置为None?

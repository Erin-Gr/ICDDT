...
# 配置在UB.yaml上，需要修改的输入更新吗
def load_config_files(file_path, args):
    '''加载yaml配置文件，并将传入的参数合并到配置字典中，优先级高于yaml文件中的设置'''
    file_path = file_path + f'{args.dataset}.yaml'
...
def show_args_info(args):
    '''通过分类和颜色化的方式，以结构化的形式将参数信息显示在控制台，增强可读性和可视化效果'''
    pad = 26
...
def check_output_path(output_dir):
    '''确保给定的输出目录存在。如果不存在，它会创建该目录，并打印一条确认消息'''
...
def data_partition(dataset_name):
    '''划分数据集得到train, valid, extra_for_test, test, userset, itemset, behaset;
        train: {u:[[i,b], [i,b], ...]; u:...}'''
    userset = set()
    itemset = set()
...
#     Early stops the training if the test metrics doesn't improve after a given patience
#     '''
#     def __init__(self, patience=20, verbose=True, delta=1e-5, save_path="./output", logger=None):
#         '''
#         Args:
#         :param patience: How many times have to wait after last time Rec. metrics improved
#                          Default: 20
...
#                 print(f'NDCG@10 metrics performed in this epoch exceeds from ({self.ndcg_10_max:.4f} --> {ndcg_10:.4f}), '
#                       f'reset the counter...')
#                 if self.logger:
#                     self.logger.info(f'NDCG@10 metrics performed in this epoch exceeds from ({self.ndcg_10_max:.4f} --> {ndcg_10:.4f}), '
#                         f'reset the counter...')
#             else:
#                 print(f'HR@10 metrics performed in this epoch exceeds from ({self.hr_10_max:.4f} --> {hr_10:.4f}), '
#                       f'reset the counter...')
#                 if self.logger:
#                     self.logger.info(f'HR@10 metrics performed in this epoch exceeds from ({self.hr_10_max:.4f} --> {hr_10:.4f}), '
#                         f'reset the counter...')
#         else:
#             self.counter += 1
#             print(f'EarlyStopping counter: {self.counter} out of {self.patience}')
...
#             print(f'HR@10 metrics has increased from ({self.hr_10_max:.4f} --> {hr_10:.4f}). Saving model ...')
#             if self.logger:
#                 self.logger.info(f'NDCG@10 metrics has increased from ({self.ndcg_10_max:.4f} --> {ndcg_10:.4f})')
#                 self.logger.info(f'HR@10 metrics has increased from ({self.hr_10_max:.4f} --> {hr_10:.4f}). Saving model ...')

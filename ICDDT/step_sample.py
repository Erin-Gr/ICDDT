...
class ScheduleSampler(ABC):
    """
    扩散的时间步分布，旨在减少优化目标的方差
    
    默认：无偏重要性采样，此时优化目标的均值不变
    
    但 子类可覆盖sample()来改变重采样项的加权方式，从而使得目标发生实际变化
    """
    ...
    def sample(self, batch_size, device):
        ...
        返回元组: (timesteps, weights)
                 - timesteps:时间步索引张量
                 - weights: 权重张量，用于缩放产生的损失。
        """
        w = self.weights() 
        p = w / np.sum(w)
...
#     def weights(self):
#         weights = np.random.beta(self.alpha, self.beta, self.num_timesteps)
#         # 继承ScheduleSampler,所以可以不归一化权重

...
class MultiHeadDiffAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # attn_mask = torch.tril(torch.ones(T, T, device=q.device)).unsqueeze(0).unsqueeze(0)
        # att1 = att1.masked_fill(attn_mask == 0, -1e9) # float('-inf'))
        # att2 = att2.masked_fill(attn_mask == 0, -1e9) # float('-inf'))
...
class FFNConditionAwareConnection(nn.Module):
    ...
    def forward(self, x, sublayer, condition):
        ...
        if condition is not None:
            ...
            # return x + self.dropout(sublayer(gate_att.unsqueeze(1) * self.adnorm(x))) # 20250108
            # return x + self.dropout(gate_att.unsqueeze(1) * sublayer(self.norm(x))) # 原来的
        else:
            return x + self.dropout(sublayer(self.norm(x)))
